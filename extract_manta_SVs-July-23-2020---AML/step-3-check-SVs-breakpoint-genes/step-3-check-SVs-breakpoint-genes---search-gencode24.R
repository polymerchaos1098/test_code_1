...
data_folder="/projectsp/foran/Team_Chan/oncocytomas_SV_Hua_Job/oncocytomas/codes/extract_manta_SVs-July-6-2020---oncocytomas/result/somaticSV-analysis-and-summary/"
...
save_folder="/projectsp/foran/Team_Chan/oncocytomas_SV_Hua_Job/oncocytomas/codes/extract_manta_SVs-July-6-2020---oncocytomas/result/somaticSV-analysis-and-summary/"
...
chr_list = c(paste("chr", c(1:22), sep=""), "chrX", "chrY", "chrM")
chr_list
...
gene_coordinates_folder = "/projectsp/foran/Team_Chan/oncocytomas_SV_Hua_Job/oncocytomas"
gene_coordinates = read.csv(paste0(gene_coordinates_folder, "/", "EnsDb.Hsapiens.v86", ".csv"), header=TRUE, sep=",")
head(gene_coordinates); dim(gene_coordinates);
 
#
gencode24 = read.csv(paste0(gene_coordinates_folder, "/", "gencode24", ".csv"), header=TRUE, sep=",")
head(gencode24); dim(gencode24); 

gencode24 = data.frame(gencode24, gene_id_short = "-")
gencode24$gene_id_short = as.character(gencode24$gene_id_short)

#
for (i in 1:nrow(gencode24)) {
    gencode24$gene_id_short[i] = unlist(strsplit(as.character(gencode24$gene_id[i]), "\\."))[1]
}
head(gencode24); dim(gencode24); 

#gene_coordinates$start = as.numeric(gene_coordinates$start)
#gene_coordinates$end = as.numeric(gene_coordinates$end)
...
#for (i in 38:38) {
	...
	pos = str_locate(tmp, "chr")
	tmp = substr(tmp, pos[1], nchar(tmp)) 
	pos1 = str_locate(tmp, "\\[")
	pos2 = str_locate(tmp, "]")
	if (!is.na(pos1[1])) {
	...
	check_chr = substr(tmp[1], str_locate(tmp[1], "chr")[2] + 1, nchar(tmp[1])) 
	check_chr
	...
	if (nrow(target)>0) {
		...
		# check back gencode24 for location of gene
		gene_list = c()
		coordinate_list = c()
		
		k = 1
		for (j in 1:nrow(target)) {
		    tmp = subset(gencode24, trimws(as.character(gene_id_short))==trimws(as.character(target$gene_id[j])))
			if (nrow(tmp)>0) {
			    if (k==1) {
				  coordinate_list = paste0(target$symbol[j], "-", target$gene_id[j], "-", tmp$seqnames, ":", tmp$start[1], "-", tmp$end[1])
				  gene_list = target$symbol[j]
				} else {
				  coordinate_list = paste0(coordinate_list, ";", paste0(target$symbol[j], "-", target$gene_id[j], "-", tmp$seqnames, ":", tmp$start[1], "-", tmp$end[1]))
				  gene_list = paste0(gene_list, ";", target$symbol[j])
				}
				k = k + 1
			}
		}
		
		if (!is.null(coordinate_list)) {
		    somatic_data$whole_coordinate_gene_1[i] = as.character(coordinate_list)
		    somatic_data$BreakPoint_Gene_1[i] = as.character(gene_list)
	    }
	...
	if (nrow(target)>0) {
		...
		if (!is.null(coordinate_list)) {
		    somatic_data$whole_coordinate_gene_2[i] = as.character(coordinate_list)
		    somatic_data$BreakPoint_Gene_2[i] = as.character(gene_list)
	    }
...
#
write.csv(somatic_data, paste0(save_folder, "gencode24---Breakpoint-gene---Oncocytomas---ALL_PASS_somaticSV_candidates", ".csv"), row.names = FALSE)

####
# load the data above
somatic_data = read.csv(paste0(save_folder, "/", "gencode24---Breakpoint-gene---Oncocytomas---ALL_PASS_somaticSV_candidates", ".csv"), header=TRUE, sep=",")
head(somatic_data); dim(somatic_data); 
...
#
write.csv(res, paste0(save_folder, "involved-COSMIC-gene-SVs---gencode24---Breakpoint-gene---Oncocytomas---ALL_PASS_somaticSV_candidates", ".csv"), row.names = FALSE)


# check if close to CCND1
COSMIC_data = read.csv(paste0(save_folder, "/", "involved-COSMIC-gene-SVs---gencode24---Breakpoint-gene---Oncocytomas---ALL_PASS_somaticSV_candidates", ".csv"), header=TRUE, sep=",")
...
#9437       11 69641087 69654474 13388      + ENSG00000110092     CCND1
#       gene_biotype seq_coord_system symbol
#9437 protein_coding       chromosome  CCND1
CCND1_coordinate_gencode24 = subset(gencode24, gene_id_short=="ENSG00000110092")
CCND1_coordinate_gencode24
#     seqnames    start      end width strand           gene_id   gene_id_short
#3748    chr11 69641087 69654474 13388      + ENSG00000110092.3 ENSG00000110092

# pickup chromosome 11
target_data = data.frame(COSMIC_data, CCND1_chr = "chr11", CCND1_start = "69641087", CCND1_end = "69654474", chr_1="-", pos_1="-", chr_2="-", pos_2="-", same_chr1="No", same_chr2="No", distance_1="-", distance_2="-", distance_3="-", distance_4="-")
...
write.csv(target_data, paste0(save_folder, "compare-CCND1---involved-COSMIC-gene-SVs---gencode24---Breakpoint-gene---Oncocytomas---ALL_PASS_somaticSV_candidates", ".csv"), row.names = FALSE)
